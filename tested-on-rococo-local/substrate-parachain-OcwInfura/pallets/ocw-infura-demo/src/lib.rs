...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		pub fn submit_number_signed(origin, number: u64) -> DispatchResult {
			...
			frame_support::runtime_print!("submit_number_signed: ({}, {:?})", number, who);
		...
		pub fn submit_number_unsigned(origin, number: u64) -> DispatchResult {
			...
			frame_support::runtime_print!("submit_number_unsigned: {}", number);
			Self::append_or_replace_number(number);
			...
			_signature: T::Signature) -> DispatchResult
		{
			...
			frame_support::runtime_print!("submit_number_unsigned_with_signed_payload: ({}, {:?})", number, public);
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			frame_support::runtime_print!("Entering off-chain worker");
...
impl<T: Config> Module<T> {
	...
	fn append_or_replace_number(number: u64) {
		Numbers::mutate(|numbers| {
			...
			frame_support::runtime_print!("Number vector: {:?}", numbers);
	...
	fn fetch_github_info() -> Result<(), Error<T>> {
		...
		if let Some(Some(gh_info)) = s_info.get::<GithubInfo>() {
			...
			frame_support::runtime_print!("cached gh-info: {:?}", gh_info);
	...
	fn fetch_n_parse() -> Result<GithubInfo, Error<T>> {
		let resp_bytes = Self::fetch_from_remote().map_err(|e| {
			...
			frame_support::runtime_print!("fetch_from_remote error: {:?}", e);
		...
		//debug::info!("{}", resp_str);
		frame_support::runtime_print!("{}", resp_str);
	...
	fn fetch_from_remote() -> Result<Vec<u8>, Error<T>> {
		...
		frame_support::runtime_print!("sending request to: {}", HTTP_REMOTE_REQUEST);


		// Initiate an external HTTP GET request. This is using high-level wrappers from `sp_runtime`.
		let request = rt_offchain::http::Request::new(HTTP_REMOTE_REQUEST);
		...
		// 	params: [],
		// }
		//curl -X POST -H "Content-Type: application/json" --data '{"jsonrpc": "2.0", "id": 1, "method": "eth_blockNumber", "params": []}' "https://mainnet.infura.io/v3/02d0bb022c1548718bf9a510d15ea440"
		//let body = "{\"jsonrpc\":\"2.0\", \"id\":\"1\", \"method\":\"eth_blockNumber\",\"params\":[]}";
		let pending = request
		    .method(sp_runtime::offchain::http::Method::Post)
			.add_header("Content-Type", CONTENT_TYPE)
			// .body(vec![ "{'jsonrpc': '2.0', 'id': 1, 'method': 'eth_blockNumber', 'params': []}"])
			.body(vec![r#"{"jsonrpc": "2.0", "id": 1, "method": "eth_blockNumber"}"#])
			// .body(vec![body])
			.deadline(timeout) // Setting the timeout time
			.send() // Sending the request out by the host
			.map_err(|_| <Error<T>>::HttpFetchingError)?;

		//debug::info!("sending request to: {:#?}", pending);
		frame_support::runtime_print!("sending request to: {:#?}", pending);
		...
		if response.code != 200 {
			...
			frame_support::runtime_print!("Unexpected http request status code: {}", response.code);
	...
	fn offchain_signed_tx(block_number: T::BlockNumber) -> Result<(), Error<T>> {
		...
		if let Some((acc, res)) = result {
			if res.is_err() {
				...
				frame_support::runtime_print!("failure: offchain_signed_tx: tx sent: {:?}", acc.id);
		...
		frame_support::runtime_print!("No local account available");
	...
	fn offchain_unsigned_tx(block_number: T::BlockNumber) -> Result<(), Error<T>> {
		...
		SubmitTransaction::<T, Call<T>>::submit_unsigned_transaction(call.into()).map_err(|_| {
			...
			frame_support::runtime_print!("Failed in offchain_unsigned_tx");
	...
	fn offchain_unsigned_tx_signed_payload(block_number: T::BlockNumber) -> Result<(), Error<T>> {
		...
		if let Some((_, res)) = signer.send_unsigned_transaction(
			...
			return res.map_err(|_| {
				...
				frame_support::runtime_print!("Failed in offchain_unsigned_tx_signed_payload");
